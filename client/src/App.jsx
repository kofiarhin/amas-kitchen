...
function Checkout({ bootstrap, cart, dispatch }) {
  const [form, setForm] = useState({ customerName: "", phone: "", email: "", line1: "", line2: "", city: "London", instructions: "", deliveryArea: "", notes: "" });
...
function PublicApp() {
  ...
  return <main><nav className="nav"><a href="#top" className="brand">Ama’s <span>Kitchen</span></a><div className="nav-links"><a href="#menu">Menu</a><a href="#how">How it works</a></div><a href="#checkout" className="basket-link"><ShoppingBag size={18} />Basket <span>{cart.lines.length}</span></a></nav>
    <header className="hero" id="top"><div className="hero-copy"><p className="eyebrow">London delivery kitchen</p><h1>Food with the warmth of home.</h1><p>Bold West African favourites, cooked fresh and delivered straight to your door.</p><div className="hero-actions"><a href="#menu" className="primary-link">Order today <ArrowRight size={18} /></a><span><Clock size={18} /> Monday–Saturday</span></div></div><div className="hero-visual"><img src={heroImage} alt="A freshly prepared dish from Ama's Kitchen" /><div className="delivery-note"><MapPin size={20} weight="fill" /><span><b>Delivery only</b>Choose your area at checkout</span></div></div></header>
    ...
    <section className="how" id="how"><p className="eyebrow">Simple by design</p><div className="how-grid"><h2>Pick a plate.<br />We’ll handle the rest.</h2><ol><li><span>01</span><div><b>Choose your food</b><p>Build your plate and check the minimum as you go.</p></div></li><li><span>02</span><div><b>Tell us where</b><p>Select an active delivery area and share clear directions.</p></div></li><li><span>03</span><div><b>Pay at your door</b><p>Your order arrives ready to enjoy. Payment is cash only.</p></div></li></ol></div></section>
    <footer><a href="#top" className="brand">Ama’s <span>Kitchen</span></a><p>Fresh food. Immediate delivery. Monday–Saturday.</p>{bootstrap && <a href={`tel:${bootstrap.supportPhone}`}>{bootstrap.supportPhone}</a>}</footer>
  </main>;
}
