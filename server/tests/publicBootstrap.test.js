...
test("builds public menu grouped in configured order with availability", async () => {
  const models = {
    ...
    DeliveryZone: { find: jest.fn(() => ({ sort: jest.fn().mockResolvedValue([{ name: "Lapaz", sortOrder: 1 }]) })) },
    Settings: { findOne: jest.fn().mockResolvedValue({ orderingEnabled: true, minimumOrder: 2000, supportPhone: businessConfig.supportPhone, supportEmail: businessConfig.supportEmail }) },
  };

  const result = await buildPublicBootstrap(models, { deliveryFeePence: 300 }, new Date("2026-06-22T10:00:00Z"));

  expect(result).toMatchObject({
    business: { city: "Accra", area: "Lapaz", currency: "GHS" },
    ordering: { canOrder: true, code: "ORDERING_OPEN" },
    deliveryFee: 300,
    minimumOrder: 2000,
    deliveryZones: [{ name: "Lapaz", sortOrder: 1 }],
  });
